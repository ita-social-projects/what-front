...
import styles from './list-of-homework.scss';
import {List} from "@components/list";
import {Table} from "@components/table";

export const ListOfHomework = () => {
  const history = useHistory();
  const pagPage = useLocation();
  const paginationPage = pagPage.state ? pagPage.state.paginationPage.paginationPage : 1;

  const [visibleHomeworks, setVisibleHomeworks] = useState([]);

  const [homeworksPerPage, sethomeworksPerPage] = useState(9);
  ...
  return (
    <div className="container pt-5">
      <div className="row justify-content-between align-items-center mb-3">
        <h2 className="col-6">Homework</h2>
        <span className="col-2 text-right">{visibleCourses.length} of {filteredCourses.length} homeworks</span>
