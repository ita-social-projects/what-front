...
export const ListOfMentors = () => {
  ...
  const [loadActiveMentors, loadAllMentors, dispatchAddAlert] = useActions([
    fetchActiveMentors,
    fetchMentors,
    addAlert,
  ]);
  ...
  useEffect(() => {
    if (isShowDisabled && allMentors.length && !areAllMentorsLoading) {
      ...
      setFilteredMentorList(
        disabledMentors.map((mentor, index) => ({ index, ...mentor }))
      );
    }
    if (!isShowDisabled && activeMentors.length && !areActiveMentorsLoading) {
      setFilteredMentorList(
        activeMentors.map((mentor, index) => ({ index, ...mentor }))
      );
    }
    setSortingCategories(INITIAL_CATEGORIES);
    setVisibleMentors(
      filteredMentorList.slice(indexOfFirstMentor, indexOfLastMentor)
    );
  }, [
    activeMentors,
    areActiveMentorsLoading,
    allMentors,
    areAllMentorsLoading,
    isShowDisabled,
  ]);
  ...
  const mentorList = () => {
    const mentors = visibleMentors.map(({ id, firstName, lastName, email }) => (
      <tr
        onClick={() => mentorDetails(id)}
        key={id}
    ...
    <div className="container pt-5">
      ...
      <div className="row mr-0">
        <div className="col-12 card shadow p-3 mb-5 bg-white ml-2 mr-2">
          ...
          <WithLoading
            isLoading={areActiveMentorsLoading || areAllMentorsLoading}
