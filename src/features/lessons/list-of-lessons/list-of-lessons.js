...
export const ListOfLessons = () => {
  ...
  const INITIAL_CATEGORIES = [
    ...
    },
    {
      ...
      name: 'lessonDate',
      sortedByAscending: false,
      tableHead: 'lessonDate',
    },
    {
      id: 2,
      name: 'lessonTime',
      sortedByAscending: false,
      tableHead: 'lessonTime',
    },
  ];
  ...
  const nextPage = useCallback(
    (pageNumber) => {
      const totalPages = Math.ceil(
        filteredLessonsList?.length / lessonsPerPage
      );
  ...
  const getLessonsList = useCallback(() => {
    const lessonsList = visibleLessonsList.map((lesson) => (
      <tr
        id={lesson.id}
        key={lesson.id}
        onClick={() => lessonDetails(lesson.id)}
    ...
    if (
      (!lessonsList.length && !lessonsList.length) ||
  ...
  const changeCountVisibleItems = useCallback(
    (newNumber) => {
